...
- Что будем считать успехом итерации с точки зрения бизнеса:
	- Снижение доли просроченных товаров на 20%
	- Снижение дефицита (out-of-stock) на 15%

#### 1.2. Бизнес-требования и ограничения  

- Краткое описание БТ и ссылки на детальные документы с бизнес-требованиями:
	- Прогноз спроса на 14 дней вперед
	- Делать прогноз по каждому товару в каждой торговой точке на каждый день
...
- Описание бизнес-процесса пилота, насколько это возможно - как именно мы будем использовать модель в существующем бизнес-процессе?    
	- Сейчас менеджеры вручную формируют заявки на закупку товаров, опираясь на интуицию, сезон и прошлый опыт. Заказы передаются в ERP-систему, оттуда — на склады и поставки. Как будет работать во время пилота:
        ...
        - Менеджеры одобряют рекомендаванный закза
        - Далее идет стандартный процесс — поставка, продажа, мониторинг остатков
        - После 2 недель сбора данных — сравниваем реальный спрос с прогнозом
...
| Ограничения   | Ограниченные вычислительные ресурсы и доступ к облакам                         | Будет использоваться минимальный клауд                        |
...
- Этап 1 — Подготовка и анализ данных
    - Анализ частоты продаж по товару и магазину
    - Обнаружение пропусков, аномалий, дубликатов
    - Выявление сезонности и трендов
- Этап 2 — Построение прогнозной модели
    - Определение целевой переменной
    - Формирование обучающей выборки
    - Выбор модели и оценка качества
- Этап 3 — Интерпретация и согласование
    - Интерпретация фичей с помощью SHAP
    - Визуализация примеров (где модель работает / не работает)
    - Согласование важных признаков с бизнесом
- Этап 4 — Интеграция бизнес-правил
    - Фиксация допустимого диапазона заказов
    - Исключение товаров, которые нельзя заказывать ежедневно
    - Ограничения на максимальный объем заказов
- Этап 5 — Инференс модели
    - Автоматизация расчета прогноза по расписанию (каждые 2 недели)
    - Генерация витрины прогнозов SKU × магазин × день
    - Сохранение результатов в целевую таблицу
- Этап 6 — Оптимизатор заказов
    - На основе прогноза и бизнес-правил рассчитывается заказ
    - Алгоритмом выступает минимизация потерь при заданных ограничениях склада и логистики
- Этап 7 — Подготовка финального отчета
    - Финальные метрики модели
    - Отчет об улучшении качества планирования (до/после)
...
- **SLI:** 99% успешных ежедневных запусков
